...
def add_data(request):
    ...
    #     {
    #         "country_name": "Guyana",
    #         "cities": ["Adelphi ", "Albion", "Betsy Ground", "	Bush Lot", "Barima Waini", "Pomeroon-Supenaam",
    #                    ]
    #     },
    #     {
    #         "country_name": "Peru",
    #         "cities": ["Bagua", "Luya", "Aija", "Casma", "Pallasca", "Andahuaylas",
    #                    "Castilla", "Cangallo", "Víctor Fajardo"]
    #     },
    #
    # ]
...
def get_continents(request):
    ...
    conti_serializer = ContinentSerializer(continents, many=True)
    return Response({"status":True, "data":conti_serializer.data})


@api_view(['GET'])
def get_countries(request, conti_id):
    countries = Countries.objects.filter(continent=conti_id)
    count_serializer = CountriesSerializer(countries, many=True)
    return Response({"status":True, "data":count_serializer.data})


@api_view(['GET'])
def get_cities(request, cities_id):
    cities = Cities.objects.filter(country=cities_id)
    cities_serializer = CitiesSerializer(cities, many=True)
    return Response({"status":True, "data":cities_serializer.data})
...
def get_cities_data(request):
    ...
    for city in cities:
        ...
        continent = Continents.objects.get(id=country.continent_id)
        city_detail = {"city_name": city.name, "country_name": country.name, "continent_name": continent.name}
        data.append(city_detail)
...
def add_city(request):
    ...
    Cities.objects.create(name=city, country_id=country)
    return Response({"status":True})
